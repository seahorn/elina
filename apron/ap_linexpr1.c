...
void ap_linexpr1_fprint(FILE* stream, const ap_linexpr1_t* e)
{
...
/*
ap_coeff_t* ap_linexpr1_set_format_get_pcoeff(char* str,
					va_list* va,
					void* expr,
					bool* b)
{
  ...
  case '%':
    ...
    pcoeff = ap_linexpr1_coeffref((ap_linexpr1_t*)expr,var);
    *b = (pcoeff==NULL);
    break;
  case '@':
    pcoeff = ap_linexpr1_cstref((ap_linexpr1_t*)expr);
    *b = false;
    break;
  default:
    fprintf(stderr,
	    "ap_linexpr0_set_format_generic/ap_linexpr1_set_format_get_pcoeff: bad format string \"%s\"",str);
    abort();
  }
  return pcoeff;
}
...
bool ap_linexpr1_set_format(ap_linexpr1_t* expr, char* fmt, ...)
{
  ...
  va_start(ap,fmt);
  b = ap_linexpr0_set_format_generic(ap_linexpr1_set_format_get_pcoeff,
				  expr, fmt, &ap);
  va_end(ap);
  return b;
}
...
/* ====================================================================== */
ap_linexpr1_t ap_linexpr1_extend_environment_dimchange(const ap_linexpr1_t* expr,
