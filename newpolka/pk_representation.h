...
void poly_set_save_C(poly_t* pa, poly_t* pb);
...
*/
void poly_chernikova(ap_manager_t* man, poly_t* poly, char* msg);

/* Same as poly_chernikova, but in addition ensure normalized epsilon
   constraints. */
void poly_chernikova2(ap_manager_t* man, poly_t* poly, char* msg);

/* Same as poly_chernikova2, but in addition normalize matrices by Gauss
   elimination and sorting */
void poly_chernikova3(ap_manager_t* man, poly_t* poly, char* msg);
...
void poly_canonicalize(ap_manager_t* man, poly_t* poly);

/* Minimize the size of the representation of the polyhedron */
void poly_minimize(ap_manager_t* man, poly_t* poly);
...
static inline bool poly_is_conseps(pk_internal_t* pk, poly_t* po);
...
static inline void poly_obtain_C(ap_manager_t* man, poly_t* po, char* msg);
static inline void poly_obtain_F(ap_manager_t* man, poly_t* po, char* msg);

/* Assuming the the matrix of constraints (resp. frames) is available, sort it,
   and take care of the saturation matrices. */
void poly_obtain_sorted_C(pk_internal_t* pk, poly_t* poly);
void poly_obtain_sorted_F(pk_internal_t* pk, poly_t* poly);

/* Assuming one of the saturation matrix is available, make satC (resp. satF)
   available. */
static inline void poly_obtain_satC(poly_t* poly);
static inline void poly_obtain_satF(poly_t* poly);
...
void poly_chernikova_dual(ap_manager_t* man,
			  ...
			  char* msg,
			  bool usual);
static inline void poly_obtain_C_dual(ap_manager_t* man,poly_t* po, char* msg, bool usual);
static inline void poly_obtain_F_dual(ap_manager_t* man,poly_t* po, char* msg, bool usual);

/* ********************************************************************** */
/* III Printing */
/* ********************************************************************** */

void poly_print(FILE* stream, ap_manager_t* man, poly_t* po,
...
void poly_dump(FILE* stream, ap_manager_t* man, poly_t* po);
...
struct ap_membuf_t poly_serialize_raw(ap_manager_t* man, poly_t* a); 
