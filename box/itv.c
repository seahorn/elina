/* ********************************************************************** */
/* itv.c: abstract lattice of (unidimensional) intervals */
/* ********************************************************************** */
...
int bound_set_scalar(num_t a, const ap_scalar_t* b, int round)
{
  if (ap_scalar_infty(b)){
    ...
    return num_set_scalar(bound_numref(a),b,round);
  }
}
		      ...
		      itv_t a, const ap_interval_t* b)
{
  ...
  bound_set_scalar(a->inf,intern->set_interval_scalar,+1);
		   ...
		   itv_t itv, const ap_coeff_t* coeff)
{
  ...
  case AP_COEFF_SCALAR:
	...
	bound_set_scalar(itv->inf, scalar, -1);
  ...
  case AP_COEFF_INTERVAL:
    itv_set_interval(intern, itv, coeff->val.interval);
    return false;
    break;
  default:
    abort();
