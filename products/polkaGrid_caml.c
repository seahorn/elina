...
value camlidl_polkaGrid_ap_pkgrid_manager_alloc_loose(value _unit)
{
  ap_manager_ptr _res;
  value _vres;
...
value camlidl_polkaGrid_ap_pkgrid_manager_alloc_strict(value _unit)
{
  ap_manager_ptr _res;
  value _vres;
  ...
  camlidl_ctx _ctx = &_ctxs;
  /* begin user-supplied calling sequence */
_res = ap_pkgrid_manager_alloc(true);
  /* end user-supplied calling sequence */
  _vres = camlidl_c2ml_manager_ap_manager_ptr(&_res, _ctx);
  camlidl_free(_ctx);
  return _vres;
