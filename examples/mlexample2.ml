...
let ex1 (man:'a Manager.t) : 'a Abstract1.t =
  ...
  Linexpr1.set_array expr
    [|
      ((Coeff.s_of_frac 1 2), var_x);
      ((Coeff.s_of_frac 2 3), var_y)
    |]
    (Some (Coeff.s_of_int (1)))
  ;
  ...
  Linexpr1.set_array expr
    [|
      ((Coeff.s_of_float (-1.0)), var_z);
      ((Coeff.s_of_float (-2.0)), var_w)
    |]
